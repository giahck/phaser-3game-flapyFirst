...
import { CvImplementation,getUniqueKeys } from "../../models/cv/cv.model";
...
//  cv!: Cv;
  ...
  set cvText(value: string) {
    console.log(value);
    const attributeName = this.attributes[this.currentAttributeIndex] as keyof CvImplementation;
    if (typeof this.cv[attributeName] === 'string') {
      //this.cv[attributeName] = value; // Imposta il valore dell'attributo solo se è una stringa
    }
  }
nextAttribute(){
  console.log(this.cv);
...
/*   }
  ...
  if (nextControl) {
    ...
    console.warn(`Controllo ${this.currentAttribute} non trovato`);
  }
  }
  get currentAttribute() { */
   // console.log(this.attributes[this.currentAttributeIndex]);
