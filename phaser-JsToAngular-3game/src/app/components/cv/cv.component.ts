...
export class CvComponent implements OnInit {
  ...
  onPersonalContact(form: NgForm) {
    if (Object.values(form.value).some((value) => {return value !== "";})) {
     /*  Object.keys(form.value).forEach((key: string) => {
        (this.cv as any)[key as keyof CvImplementation] = form.value[key];
      }); */
      ...
      /* attivo formazione e disattivo personal e inserisco e visualizo i dati */ else {
        ...
        if (this.cv.esperienze.length === 0) {
          console.log(this.cv.esperienze.length);
    ...
    cancellaEsperienza() {
      if (this.attributesEsperienza.length > 0) {
        console.log(this.cv.esperienze.length);
        console.log(this.attributesEsperienza.length);
  ...
  addEsperienza(form: NgForm) {
    if ((form.valid &&this.countAttributesEsperienza === this.cv.esperienze.length - 1) ||this.cv.esperienze.length === 0) {
      const esperienzaArray = this.convertToAttributes(new EsperienzaImplementation());
      this.attributesEsperienza.push(esperienzaArray);
      this.countAttributesEsperienza++;
    }
  }
  onPersonalEsperienza(form: NgForm) {
    if (form.valid) {
      ...
      /* Object.keys(form.value).forEach((key: string) => {
        (this.cv.esperienze[this.countAttributesEsperienza] as any)[
          key as keyof CvImplementation
        ] = form.value[key];
      }); */
    ...
    /*   else {
        ...
        if (this.cv.formazioni.length === 0) {
          this.addFormazione();
  ...
  addFormazione() {
    const nuovaEsp = new FormazioneImplementation();
    this.attributesFormazione.push(this.convertToAttributes(nuovaEsp));
    this.countAttributesFormazione++;
  }
/*   else {
    this.switches.esp = true;
  ...
  backBlock() {
   if (this.countAttributesEsperienza > 0) {
      this.countAttributesEsperienza--;
    } else if (this.countAttributesFormazione > 0) {
      this.countAttributesFormazione--;
    }
  }
  ngOnDestroy(): void {
